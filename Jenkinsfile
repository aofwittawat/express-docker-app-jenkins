// ฟังก์ชันสำหรับส่ง notification ไปยัง n8n webhook
def sendN8nWebhook(String stageName, String status) {
    withCredentials([string(credentialsId: 'n8n-webhook', variable: 'N8N_WEBHOOK_URL')]) {
        ...
        try {
            // พยายามใช้ httpRequest plugin ก่อน
            ...
            echo "n8n webhook sent successfully (${status})"
        } catch (err) {
            // ถ้า httpRequest ไม่พร้อมใช้งาน ใช้ Java URLConnection แทน
            echo "httpRequest plugin not available: ${err}. Using Java URLConnection..."
            try {
                ...
                echo "n8n webhook sent via URLConnection (${status}), response code: ${rc}"
            } catch (e2) {
                echo "Failed to send n8n webhook (${status}): ${e2}"
            }
        }
    }
}
...
pipeline {
    ...
    environment {
        ...
        DOCKER_REPO               = "aofwittawat/express-docker-app-jenkins"
    ...
    stages {
        ...
        stage('Deploy Local') {
            ...
            post {
                success {
                    script {
                        sendN8nWebhook('Deploy Local', 'success')
                    }
                }
            }
        }
    ...
    // Post actions สำหรับทั้ง pipeline
    post {
        always {
        ...
        failure {
            // ส่ง notification ไปยัง n8n เมื่อ pipeline ล้มเหลว
            script {
                sendN8nWebhook('Pipeline', 'failed')
            }
        }
    }
}
